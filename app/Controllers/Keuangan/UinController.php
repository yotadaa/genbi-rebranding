...
public function kegiatan(Request $request, Response $response): void
    {
        ...
        $response->html($this->view->renderWithLayout('keuangan/bendahara/uin/kegiatan/index.php', 'layouts/bendaharaKomsatUIN.php', [
            'activeMenu' => 'kegiatan',
            'kegiatan' => $kegiatan,
            'title' => 'Kegiatan Komsat UIN'
        ]));
    }

    public function tambahKegiatan(Request $request, Response $response): void
    {
        $response->html($this->view->renderWithLayout('keuangan/bendahara/uin/kegiatan/form.php', 'layouts/bendaharaKomsatUIN.php', [
            'activeMenu' => 'kegiatan',
            'isEdit' => false,
            'kegiatan' => [],
            'title' => 'Tambah Kegiatan Komsat UIN'
        ]));
    }
    ...
    public function storeKegiatan(Request $request, Response $response): void
    {
        ...
        $tingkat = trim((string) ($_POST['tingkat'] ?? 'uin'));
        ...
        if ($tingkat !== 'uin') {
            $errors['tingkat'] = 'Akses ditolak: Anda hanya dapat menambahkan kegiatan untuk tingkat Komsat UIN.';
        }

        if (!empty($errors)) {
            \App\Core\Session::flash('errors', $errors);
            \App\Core\Session::flash('old', $_POST);
            $response->redirect('/keuangan/bendahara/uin/kegiatan/tambah');
            return;
        }
    ...
    public function editKegiatan(Request $request, Response $response, array $args): void
    {
        $id = $args['id'] ?? null;
        $db = \App\Core\Database::connection();
        $stmt = $db->prepare("SELECT * FROM tbl_kegiatan_keuangan WHERE id = ? AND tingkat = 'uin'");
        $stmt->execute([$id]);
        $kegiatan = $stmt->fetch();

        if (!$kegiatan) {
            \App\Core\Session::flash('swal_error', 'Kegiatan tidak ditemukan atau bukan tingkat UIN.');
            $response->redirect('/keuangan/bendahara/uin/kegiatan');
            return;
        }
        ...
        $response->html($this->view->renderWithLayout('keuangan/bendahara/uin/kegiatan/form.php', 'layouts/bendaharaKomsatUIN.php', [
            ...
            'isEdit' => true,
            'kegiatan' => $kegiatan,
            'title' => 'Edit Kegiatan Komsat UIN'
        ]));
    }
    ...
    public function updateKegiatan(Request $request, Response $response, array $args): void
    {
        ...
        $db = \App\Core\Database::connection();
        $stmt = $db->prepare("SELECT id FROM tbl_kegiatan_keuangan WHERE id = ? AND tingkat = 'uin'");
        $stmt->execute([$id]);
        if (!$stmt->fetch()) {
        ...
        if ($tingkat !== 'uin') {
            $errors['tingkat'] = 'Akses ditolak: Anda hanya dapat mengubah kegiatan untuk tingkat Komsat UIN.';
        }

        if (!empty($errors)) {
            \App\Core\Session::flash('errors', $errors);
            \App\Core\Session::flash('old', $_POST);
            $response->redirect('/keuangan/bendahara/uin/kegiatan/edit/' . $id);
            return;
        }
    ...
    public function hapusKegiatan(Request $request, Response $response, array $args): void
    {
        ...
        $db = \App\Core\Database::connection();
        $stmt = $db->prepare("DELETE FROM tbl_kegiatan_keuangan WHERE id = ? AND tingkat = 'uin'");
        $stmt->execute([$id]);
...
public function transaksi(Request $request, Response $response): void
    {
        ...
        $response->html($this->view->renderWithLayout('keuangan/bendahara/uin/transaksi/index.php', 'layouts/bendaharaKomsatUIN.php', [
            'activeMenu' => 'transaksi',
            'dummyData' => $mappedData,
            'title' => 'Transaksi Keuangan Komsat UIN'
        ]));
    }
    ...
    public function transaksiCreate(Request $request, Response $response): void
    {
        ...
        $response->html($this->view->renderWithLayout('keuangan/bendahara/uin/transaksi/form.php', 'layouts/bendaharaKomsatUIN.php', [
            'activeMenu' => 'transaksi',
            'title' => 'Tambah Transaksi UIN',
    ...
    public function transaksiStore(Request $request, Response $response): void
    {
        ...
        $db = \App\Core\Database::connection();
        $stmtProf = $db->prepare("SELECT nama_bendahara, tahun_periode_awal, tahun_periode_akhir FROM tbl_profil_bendahara WHERE user_id = ? AND tempat = 'uin'");
                ...
                if (isset($_FILES['bukti_file']) && $_FILES['bukti_file']['error'] === UPLOAD_ERR_OK) {
                    ...
                    if (!$isPdf && !$isImg) {
                        ...
                        if ($isPdf && $fileSize > 10 * 1024 * 1024) {
                            ...
                            $newFileName = 'trx_uin_' . time() . '_' . uniqid() . '.' . $fileExt;
        ...
        if (!empty($errors)) {
            ...
            \App\Core\Session::flash('old', $_POST);
            $response->redirect('/keuangan/bendahara/uin/transaksi/tambah');
            return;
        }
    ...
    public function transaksiEdit(Request $request, Response $response, array $args): void
    {
        ...
        $db = \App\Core\Database::connection();
        $stmt = $db->prepare("SELECT * FROM tbl_transaksi_uin WHERE id = ?");
        $stmt->execute([$id]);
        $trx = $stmt->fetch();
    ...
    public function transaksiUpdate(Request $request, Response $response, array $args): void
    {
        ...
        if (!empty($errors)) {
            ...
            \App\Core\Session::flash('error_fields', $error_fields);
            \App\Core\Session::flash('old', $_POST);
            $response->redirect('/keuangan/bendahara/uin/transaksi/edit/' . $id);
            return;
        }
    ...
    public function transaksiHapus(Request $request, Response $response, array $args): void
    {
        $id = $args['id'] ?? null;
        $db = \App\Core\Database::connection();
        $stmt = $db->prepare("DELETE FROM tbl_transaksi_uin WHERE id = ?");
        $stmt->execute([$id]);
