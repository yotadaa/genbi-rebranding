...
$actionUrl = $isEdit
    ? '/keuangan/bendahara/uin/kegiatan/edit/' . ($kegiatan['id'] ?? '')
    : '/keuangan/bendahara/uin/kegiatan/tambah';
?>
<div class="max-w-3xl mx-auto pb-12">
    ...
    <div class="mb-10 md:mb-14 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-6">
        <div>
            ...
            <p class="text-[15px] text-slate-500 mt-4 max-w-2xl leading-relaxed">Isi formulir di bawah ini dengan lengkap dan benar untuk Komsat UIN.</p>
        </div>
        <a href="/keuangan/bendahara/uin/kegiatan" class="inline-flex items-center justify-center px-6 py-3 bg-white border border-slate-200/60 text-slate-700 rounded-xl text-[13px] font-semibold hover:bg-slate-50 hover:shadow-md transition-all shadow-sm mt-2">
            Kembali
        </a>
    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <select id="tingkat" name="tingkat" class="w-full px-5 py-3.5 bg-slate-50 border <?= $hasError('tingkat') ? $errorClass : $normalClass ?> rounded-xl outline-none transition-all text-[13px]">
                            ...
                            <option value="uin" <?= ($val('tingkat') === 'uin' || !$isEdit) ? 'selected' : '' ?>>Komsat UIN</option>
                            <option value="unja" <?= $val('tingkat') === 'unja' ? 'selected' : '' ?>>Komsat UNJA</option>
                        </select>
                        <p class="mt-2 text-[11px] font-medium text-slate-400">Anda hanya dapat menambahkan kegiatan untuk tingkat Komsat UIN.</p>
                    </div>
                </div>
