...
<div class="max-w-4xl mx-auto pb-12">
    ...
    <div class="mb-10 md:mb-14 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-6">
        <div>
            <div class="text-[10px] md:text-xs font-bold text-slate-500 uppercase tracking-[0.2em] mb-4">TAMBAH DATA</div>
            <h1 class="text-4xl md:text-[2.75rem] font-serif-title font-medium text-slate-900 tracking-tight leading-tight">Transaksi Baru.</h1>
            <p class="text-[15px] text-slate-500 mt-4 max-w-2xl leading-relaxed">Isi formulir di bawah untuk mencatat transaksi pengeluaran atau pemasukan baru.</p>
        </div>
            ...
            <div class="p-8 md:p-10 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal_transaksi" required value="<?= $e($old['tanggal_transaksi'] ?? date('Y-m-d')) ?>" class="w-full px-5 py-3.5 bg-slate-50 border rounded-xl outline-none transition-all text-[13px] <?= $hasError('tanggal_transaksi') ?>">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="nominal_display" required value="<?= $e($old['nominal'] ?? '') ?>" placeholder="Misal: 500.000" class="w-full px-5 py-3.5 bg-slate-50 border rounded-xl outline-none transition-all text-base font-semibold text-slate-900 <?= $hasError('nominal') ?>">
                ...
                <div>
                    ...
                    <textarea name="keterangan_transaksi" rows="3" required placeholder="Detail pengeluaran atau pemasukan..." class="w-full px-5 py-3.5 bg-slate-50 border rounded-xl outline-none transition-all text-[13px] resize-none <?= $hasError('keterangan_transaksi') ?>"><?= $e($old['keterangan_transaksi'] ?? '') ?></textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        ...
                        <input type="text" name="alokasi_dana" list="alokasi_list" required value="<?= $e($old['alokasi_dana'] ?? '') ?>" placeholder="Pilih atau ketik kustom..." class="w-full px-5 py-3.5 bg-slate-50 border rounded-xl outline-none transition-all text-[13px] <?= $hasError('alokasi_dana') ?>">
                    ...
                    <div>
                        ...
                        <input type="text" name="sumber_dana" value="<?= $e($old['sumber_dana'] ?? '') ?>" placeholder="Misal: Kas GenBI, Sponsorship, dll" class="w-full px-5 py-3.5 bg-slate-50 border rounded-xl outline-none transition-all text-[13px] <?= $hasError('sumber_dana') ?>">
                    </div>
                </div>
                ...
                <div class="border-t border-slate-100/80 pt-8 mt-8">
                    ...
                    <div class="flex items-center gap-6 mb-5">
                        <label class="inline-flex items-center cursor-pointer group">
                            <input type="radio" name="input_mode" value="file" class="w-4 h-4 text-slate-900 border-slate-300 focus:ring-slate-900" <?= (isset($old['input_mode']) && $old['input_mode'] === 'link') ? '' : 'checked' ?>>
                            <span class="ml-2.5 text-[13px] text-slate-700 font-semibold group-hover:text-slate-900 transition-colors">Upload File</span>
                        </label>
                        <label class="inline-flex items-center cursor-pointer group">
                            <input type="radio" name="input_mode" value="link" class="w-4 h-4 text-slate-900 border-slate-300 focus:ring-slate-900" <?= (isset($old['input_mode']) && $old['input_mode'] === 'link') ? 'checked' : '' ?>>
                            <span class="ml-2.5 text-[13px] text-slate-700 font-semibold group-hover:text-slate-900 transition-colors">Link Google Drive</span>
                        </label>
                    </div>
                    ...
                    <div id="link-input-group" class="<?= (isset($old['input_mode']) && $old['input_mode'] === 'link') ? '' : 'hidden' ?>">
                        <input type="url" name="bukti_link" value="<?= $e($old['bukti_link'] ?? '') ?>" placeholder="https://drive.google.com/..." class="w-full px-5 py-3.5 bg-slate-50 border rounded-xl outline-none transition-all text-[13px] <?= $hasError('bukti_link') ?>">
