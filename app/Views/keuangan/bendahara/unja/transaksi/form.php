...
$actionUrl = $isEdit ? "/keuangan/bendahara/unja/transaksi/edit/" . ($trx['id'] ?? '') : "/keuangan/bendahara/unja/transaksi/tambah";
...
<div class="max-w-4xl mx-auto pb-12">
    ...
    <div class="mb-10 md:mb-14 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-6">
        <div>
            ...
            <p class="text-[15px] text-slate-500 mt-4 max-w-2xl leading-relaxed">Isi formulir di bawah untuk mencatat transaksi pengeluaran atau pemasukan Komsat UNJA.</p>
        </div>
        <a href="/keuangan/bendahara/unja/transaksi" class="inline-flex items-center justify-center px-6 py-3 bg-white border border-slate-200/60 text-slate-700 rounded-xl text-[13px] font-semibold hover:bg-slate-50 hover:shadow-md transition-all shadow-sm mt-2">
            Kembali
        </a>
    </div>
            ...
            <div class="p-8 md:p-10 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <label class="relative flex items-center p-4 border border-slate-200/60 rounded-2xl cursor-pointer hover:bg-slate-50 transition-colors group">
                        ...
                        <div class="w-5 h-5 rounded-full border-2 border-slate-300 peer-checked:border-[#f97316] peer-checked:border-[6px] transition-all mr-4"></div>
                        <div>
                ...
                <div class="border-t border-slate-100/80 pt-8 mt-8">
                    ...
                    <div class="flex items-center gap-6 mb-5">
                        <label class="inline-flex items-center cursor-pointer group">
                            ...
                            <span class="ml-2.5 text-[13px] text-slate-700 font-semibold group-hover:text-[#f97316] transition-colors">Upload File</span>
                        </label>
                        <label class="inline-flex items-center cursor-pointer group">
                            ...
                            <span class="ml-2.5 text-[13px] text-slate-700 font-semibold group-hover:text-[#f97316] transition-colors">Link Google Drive</span>
                        </label>
                    </div>
                    ...
                    <div id="file-input-group" class="<?= $currentInputMode === 'link' ? 'hidden' : '' ?>">
                        ...
                        <?php if ($isEdit && $currentInputMode === 'file' && !empty($trx['bukti_transaksi']) && !filter_var($trx['bukti_transaksi'], FILTER_VALIDATE_URL)): ?>
                            <p class="text-[11px] font-medium text-slate-500 mt-2">File saat ini: <a href="/uploads/keuangan/<?= htmlspecialchars($trx['bukti_transaksi']) ?>" target="_blank" class="text-[#f97316] hover:underline">Lihat Bukti</a>. (Biarkan kosong jika tidak ingin mengubah file).</p>
                        <?php endif; ?>
                    </div>
                    ...
                    <div id="link-input-group" class="<?= $currentInputMode === 'link' ? '' : 'hidden' ?>">
                        <div class="flex gap-2">
                            ...
                            <?php if ($isEdit && $currentInputMode === 'link' && !empty($trx['bukti_transaksi'])): ?>
                                <a href="<?= htmlspecialchars($trx['bukti_transaksi']) ?>" target="_blank" class="shrink-0 flex items-center justify-center px-5 py-3.5 bg-[#f97316]/10 text-[#f97316] font-semibold text-[13px] rounded-xl hover:bg-[#f97316]/20 transition-all">
                                    Buka Link
                                </a>
                            <?php endif; ?>
                        </div>
