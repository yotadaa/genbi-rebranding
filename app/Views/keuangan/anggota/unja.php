...
<div class="site-container py-8">
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-slate-900 tracking-tight">Keuangan UNJA</h1>
        <p class="text-slate-500 mt-2">Dashboard pantauan arus kas dan riwayat transaksi Keuangan UNJA GenBI Jambi.</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 flex items-center gap-4 transition-transform hover:-translate-y-1 relative overflow-hidden">
            <div class="absolute -right-4 -top-4 w-24 h-24 bg-orange-50 rounded-full opacity-50 pointer-events-none"></div>
            <div class="w-14 h-14 rounded-full bg-orange-100 text-orange-600 flex items-center justify-center shrink-0 relative z-10">
                <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
        ...
        <div class="lg:col-span-1 flex flex-col gap-6">
            <div class="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100/60 p-6">
                ...
                <div class="relative">
                    <input type="text" id="search-input" placeholder="Cari transaksi..." class="w-full pl-11 pr-4 py-3 bg-slate-50 border-transparent focus:bg-white focus:border-orange-500 rounded-xl text-sm transition-all focus:ring-4 focus:ring-orange-500/10 placeholder-slate-400">
            ...
            <div class="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100/60 p-6 flex-grow">
                ...
                <div class="flex flex-col gap-2" id="filter-container">
                    <button class="category-btn w-full text-left px-5 py-3 rounded-xl text-[13px] font-semibold transition-all bg-orange-600 text-white shadow-md shadow-orange-600/20" data-cat="Semua Kategori">Semua Kategori</button>
                    <button class="category-btn w-full text-left px-5 py-3 rounded-xl text-[13px] font-semibold transition-all bg-slate-50 text-slate-600 hover:bg-slate-100" data-cat="BPI Komsat UNJA">BPI Komsat UNJA</button>
                    <button class="category-btn w-full text-left px-5 py-3 rounded-xl text-[13px] font-semibold transition-all bg-slate-50 text-slate-600 hover:bg-slate-100" data-cat="Kewirausahaan">Kewirausahaan</button>
                    <button class="category-btn w-full text-left px-5 py-3 rounded-xl text-[13px] font-semibold transition-all bg-slate-50 text-slate-600 hover:bg-slate-100" data-cat="Lingkungan Hidup">Lingkungan Hidup</button>
                    <button class="category-btn w-full text-left px-5 py-3 rounded-xl text-[13px] font-semibold transition-all bg-slate-50 text-slate-600 hover:bg-slate-100" data-cat="Pendidikan dan Kebudayaan">Pendidikan dan Kebudayaan</button>
                    <button class="category-btn w-full text-left px-5 py-3 rounded-xl text-[13px] font-semibold transition-all bg-slate-50 text-slate-600 hover:bg-slate-100" data-cat="Pengabdian Masyarakat">Pengabdian Masyarakat</button>
                    <button class="category-btn w-full text-left px-5 py-3 rounded-xl text-[13px] font-semibold transition-all bg-slate-50 text-slate-600 hover:bg-slate-100" data-cat="Pengembangan Sumber Daya Manusia">Pengembangan SDM</button>
                    <button class="category-btn w-full text-left px-5 py-3 rounded-xl text-[13px] font-semibold transition-all bg-slate-50 text-slate-600 hover:bg-slate-100" data-cat="Publikasi dan Sosial">Publikasi dan Sosial</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100/60 overflow-hidden mb-8">
        ...
        <div class="p-5 border-t border-slate-100 flex items-center justify-between bg-slate-50/50">
            <button id="btn-prev" class="px-5 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-xl text-sm font-semibold hover:bg-slate-50 hover:text-orange-600 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-sm">
                Sebelumnya
            </button>
            <button id="btn-next" class="px-5 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-xl text-sm font-semibold hover:bg-slate-50 hover:text-orange-600 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-sm">
                Selanjutnya
            </button>
        </div>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const render = () => {
            ...
            if (currentData.length === 0) {
                ...
                currentData.forEach(t => {
                    ...
                    <td class="px-6 py-4 text-[13px] font-bold text-slate-900 group-hover:text-orange-600 transition-colors">${t.desc}</td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-[11px] font-semibold bg-orange-50 text-orange-700 border border-orange-100/50">
                            ${t.category}
                        </span>
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-center">
                        <button type="button" class="p-2 text-slate-400 hover:text-orange-600 hover:bg-orange-50 rounded-xl transition-all" onclick="showDetail(${t.id})" title="Detail">
        ...
        window.showDetail = (id) => {
            ...
            if (t.proof) {
                ...
                proofHtml = `
                <a href="${url}" target="_blank" class="inline-flex items-center gap-2 px-4 py-2 mt-2 bg-orange-50 text-orange-700 hover:bg-orange-100 hover:text-orange-800 rounded-xl text-sm font-semibold transition-all">
                ...
                if (btn) {
                    ...
                    filterContainer.querySelectorAll('.category-btn').forEach(b => {
                        if (b.dataset.cat === activeCategory) {
                            b.className = 'category-btn w-full text-left px-5 py-3 rounded-xl text-[13px] font-semibold transition-all bg-orange-600 text-white shadow-md shadow-orange-600/20';
                        } else {
