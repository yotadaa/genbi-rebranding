...
@php $comments = $comments ?? []; @endphp
@php $commentPolicy = $item['comment_policy'] ?? ['comments_enabled' => true, 'voting_enabled' => true, 'replies_enabled' => true, 'max_reply_depth' => 3]; @endphp

@php
// Helper to render comments recursively in Blade
$renderCommentNode = function (array $comment, int $depth = 0) use (&$renderCommentNode, $commentPolicy) {
  ...
  ob_start();
  @endphp
  <article class="comment-item comment-node comment-depth-{{ min($depth, 6) }}" data-comment-id="{{ $comment['id'] ?? 0 }}">
    <div class="flex items-start justify-between gap-3">
      <div>
        <h3>{{ $comment['name'] ?? 'Pembaca' }}</h3>
    ...
    <div class="comment-meta-row">
      @if ($canVote)
        <div class="comment-vote-group">
          <button type="button" class="comment-vote-btn" data-vote-up="{{ $comment['id'] ?? 0 }}">↑ <span data-vote-up-count>{{ $comment['up_votes'] ?? 0 }}</span></button>
          <button type="button" class="comment-vote-btn" data-vote-down="{{ $comment['id'] ?? 0 }}">↓ <span data-vote-down-count>{{ $comment['down_votes'] ?? 0 }}</span></button>
          <span class="comment-score" data-vote-score>{{ $comment['score'] ?? 0 }}</span>
        </div>
      @endif
      @if ($canReply)
        <button type="button" class="comment-reply-toggle" data-reply-toggle="{{ $comment['id'] ?? 0 }}">Balas</button>
      @endif
    </div>
    @if ($canReply)
      <form class="comment-form comment-reply-form hidden" data-reply-form="{{ $comment['id'] ?? 0 }}">
        <input type="hidden" name="parent_id" value="{{ $comment['id'] ?? 0 }}" />
  ...
  <section class="bg-cream py-10 md:py-16">
    <article class="article-container fade-up in-view">
      <div id="news-detail-root" data-ssr="true" class="prose-soft news-detail-content">
        @if (!empty($item['image']))
          <img class="news-detail-inline-image" src="{{ $item['image'] }}" alt="{{ $item['title'] }}" loading="lazy" onerror="this.remove()">
        @endif
        {!! $item['content'] ?? '' !!}
      </div>
      @if (!empty($item['contributor_pewarta']) || !empty($item['contributor_editor']))
        <div class="news-detail-contributors mt-10 rounded-[1.5rem] border border-neutral-900/10 bg-white/80 p-5 text-sm leading-7 text-neutral-700">
          @if (!empty($item['contributor_pewarta']))
            <p><strong>Pewarta:</strong> {{ $item['contributor_pewarta'] }}</p>
          @endif
          @if (!empty($item['contributor_editor']))
            <p><strong>Editor:</strong> {{ $item['contributor_editor'] }}</p>
          @endif
        </div>
      @endif
    </article>
    ...
    <div class="article-container fade-up in-view">
      <div class="news-engagement-grid">
        ...
        <section class="comment-card">
          ...
          @if (!empty($commentPolicy['comments_enabled']))
