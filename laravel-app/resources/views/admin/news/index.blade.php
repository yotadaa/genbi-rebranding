...
<section class="mx-auto max-w-7xl">
  ...
  <div class="mt-6">
    <section class="admin-card p-4 md:p-6">
      <form method="get" action="{{ url('/admin/news') }}" class="cms-toolbar cms-toolbar-admin" id="news-filter-form">
        ...
        <div class="admin-toolbar-row">
          <label class="cms-search">
            ...
            <input id="news-search" name="q" placeholder="Cari judul, konten, kategori..." value="{{ $filters['q'] ?? '' }}">
          </label>
      ...
      <div class="admin-toolbar-footer mt-4">
        <div class="view-toggle" role="group" aria-label="Layout mode">
          <a href="?{{ http_build_query(array_merge($filterParams, ['layout' => 'list'])) }}" class="view-toggle-btn {{ $layout === 'list' ? 'is-active' : '' }}">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path></svg>
            List
          </a>
          <a href="?{{ http_build_query(array_merge($filterParams, ['layout' => 'grid'])) }}" class="view-toggle-btn {{ $layout === 'grid' ? 'is-active' : '' }}">
                  ...
                  <tr>
                    ...
                    <td class="admin-cell-title">
                      <div class="admin-table-title">
                        ...
                        <p>{{ substr((string) ($item['date'] ?? ''), 0, 10) }}</p>
                      </div>
                    </td>
                    <td class="admin-cell-excerpt">
                      <p class="admin-table-copy news-caption-cell">{{ $item['excerpt'] }}</p>
                    </td>
                    <td class="admin-cell-thumb">
                      @if (!empty($item['photo']))
                        <img src="{{ $item['photo'] }}" class="table-thumb" alt="{{ $item['title'] }}" onerror="this.style.display='none'">
                      @else
                        <span class="admin-thumb-placeholder">No image</span>
                      @endif
                    </td>
                    <td class="admin-cell-meta"><span class="cms-pill">{{ $item['category'] }}</span></td>
                    <td class="admin-cell-status">
                      <span class="status-pill {{ strtolower($item['status'] ?? 'draft') }}">
                        {{ ucfirst($item['status'] ?? 'draft') }}
                      </span>
                    </td>
                    <td class="admin-cell-actions">
                      <div class="admin-table-actions">
                        <a class="btn btn-secondary btn-sm" href="{{ route('admin.news.edit', ['id' => (int) $item['id']]) ?? url('/admin/news/' . (int)$item['id'] . '/edit') }}">
        ...
        @else
          @foreach ($items as $item)
            <article class="admin-news-card">
              @if (!empty($item['photo']))
                <img src="{{ $item['photo'] }}" alt="{{ $item['title'] }}" onerror="this.remove()">
              @else
                <div class="admin-news-card-placeholder">No Image</div>
              @endif
              <div class="admin-news-card-content">
                <h2>{{ $item['title'] }}</h2>
                <p>{{ mb_substr($item['excerpt'], 0, 120) }}{{ mb_strlen($item['excerpt']) > 120 ? '...' : '' }}</p>
                <div class="admin-news-card-meta">
                  <span class="cms-pill">{{ $item['category'] }}</span>
                  <span class="status-pill {{ strtolower($item['status'] ?? 'draft') }}">{{ ucfirst($item['status'] ?? 'draft') }}</span>
                  <span class="text-xs text-neutral-500">{{ substr((string) ($item['date'] ?? ''), 0, 10) }}</span>
                </div>
    ...
    @if ($totalPages > 1)
      <nav class="admin-pagination mt-5" aria-label="Pagination berita" data-ssr="true">
        @if ($page > 1)
          <a class="pager-button" href="{{ url('/admin/news') }}?{{ $buildQuery($page - 1, $filterParams) }}">Sebelumnya</a>
        @else
        ...
        @for ($i = 1; $i <= $totalPages; $i++)
          ...
          @else
            <a class="pager-button" href="{{ url('/admin/news') }}?{{ $buildQuery($i, $filterParams) }}">{{ $i }}</a>
          @endif
        @endfor
        @if ($page < $totalPages)
          <a class="pager-button" href="{{ url('/admin/news') }}?{{ $buildQuery($page + 1, $filterParams) }}">Berikutnya</a>
        @else
