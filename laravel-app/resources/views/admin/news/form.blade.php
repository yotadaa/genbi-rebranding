...
$content = $isEdit ? ($item['content'] ?? '') : ''; // Assumes Blade template handles raw html when needed or JS handles it
    ...
    <form class="medium-editor-layout" id="news-editor-form" data-ssr="true" data-edit="{{ $isEdit ? '1' : '0' }}" data-item-id="{{ $itemId }}">
      <main class="medium-editor-canvas">
        ...
        <section class="story-main-block">
          ...
          <div id="news-title-field" class="story-title-field" contenteditable="true" spellcheck="true" data-placeholder="Tulis judul berita...">{{ $title }}</div>
        </section>
        <section class="story-main-block">
          <label for="news-short-content-field">News Short Content</label>
          <div id="news-short-content-field" class="story-excerpt-field" contenteditable="true" spellcheck="true" data-placeholder="Tulis caption atau ringkasan singkat untuk news list...">{{ $excerpt }}</div>
        </section>
      ...
      <aside class="editor-config-sidebar medium-config-sidebar">
        ...
        <section class="config-card medium-config-card">
          <h2>Publishing</h2>
          <div class="config-field"><span>News Publish Date</span><input class="config-input" type="date" value="{{ $date }}" /></div>
          <div class="config-field"><span>Category</span>
            <select class="config-input js-admin-custom-select" id="news-category-select">
              @foreach ($categories as $cat)
                <option value="{{ (int) $cat['category_id'] }}" {{ (int) $cat['category_id'] === (int) $categoryId ? 'selected' : '' }}>{{ $cat['category_name'] }}</option>
              @endforeach
            </select>
          </div>
        ...
        <section class="config-card medium-config-card">
          ...
          @if (!empty($photo))
            <img src="{{ $photo }}" class="config-preview" alt="Featured photo" />
          @else
          ...
          <input class="config-input mt-2" id="news-photo-url" value="{{ $photo }}" placeholder="URL foto utama" />
        </section>
        <section class="config-card medium-config-card">
          <h2>Contributors</h2>
          <div class="config-field"><span>Pewarta</span><input class="config-input" id="news-pewarta" value="{{ $pewarta }}" /></div>
          <div class="config-field"><span>Editor</span><input class="config-input" id="news-editor-name" value="{{ $editor }}" /></div>
        </section>
        <section class="config-card medium-config-card">
          <h2>SEO Information</h2>
          <div class="config-field"><span>Meta Title</span><input class="config-input" id="news-meta-title" value="{{ $metaTitle ?: $title }}" /></div>
          <div class="config-field"><span>Meta Keywords</span><textarea class="config-input" id="news-meta-keyword" rows="4">{{ $metaKeyword }}</textarea></div>
          <div class="config-field"><span>Meta Description</span><textarea class="config-input" id="news-meta-desc" rows="5">{{ $metaDesc ?: $excerpt }}</textarea></div>
        </section>
        <section class="config-card medium-config-card">
          <h2>Status</h2>
