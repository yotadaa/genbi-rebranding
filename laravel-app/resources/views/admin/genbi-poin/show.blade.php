...
<section class="mx-auto max-w-7xl">
  <header class="cms-header slide-in">
    <div>
      ...
      <p class="mt-4 max-w-2xl text-base leading-7 text-neutral-600">{{ $member ? ($member['name'] ?? '') : 'Anggota tidak ditemukan.' }}</p>
    </div>
    <div class="cms-actions">
      <a href="{{ route('admin.genbiPoin') }}" class="btn btn-secondary">Kembali</a>
      <a href="{{ route('admin.genbiPoin.add', ['team_id' => $teamId]) }}" class="btn btn-primary">Tambah Aktivitas</a>
    </div>
  </header>
  ...
  @else
    <div class="mt-6 grid gap-5 lg:grid-cols-[1fr_1.25fr]">
      <section class="admin-card p-5 md:p-6">
        ...
        <h2 class="mt-2 text-2xl font-bold text-neutral-950">{{ $member['name'] ?? '' }}</h2>
        <p class="mt-2 text-sm font-semibold text-neutral-600">{{ $member['role'] ?? '-' }}</p>
        <div class="mt-5 grid gap-3 text-sm text-neutral-700">
          <div><span class="font-bold text-blue-900">Divisi:</span> {{ $member['division'] ?? '-' }}</div>
          <div><span class="font-bold text-blue-900">Kampus:</span> {{ $member['campus'] ?? '-' }}</div>
        </div>
      </section>
      ...
      <section class="admin-card p-5 md:p-6">
        ...
        <div class="mt-4 genbi-point-summary">
          <div><span>Poin Presensi</span><strong>{{ (int) ($member['presensi_points'] ?? 0) }}</strong></div>
          <div><span>Poin Manual</span><strong>{{ (int) ($member['manual_points'] ?? 0) }}</strong></div>
          <div><span>Total Poin</span><strong>{{ (int) ($member['total_points'] ?? 0) }}</strong></div>
        </div>
      </section>
    </div>
    ...
    <section class="admin-card p-0 mt-5">
      ...
      <div class="admin-data-table-wrap">
        <table class="admin-table admin-data-table genbi-point-detail-table">
          <thead><tr><th>Event</th><th>Role</th><th>Poin</th><th>Status</th><th>Waktu</th></tr></thead>
          <tbody>
            @forelse ($presensiActivities as $activity)
              @php $status = strtolower((string) ($activity['status'] ?? 'pending')); @endphp
              <tr>
                <td class="admin-cell-title"><strong>{{ $activity['event_name'] ?? '' }}</strong><p>{{ $activity['location'] ?? '-' }}</p></td>
                <td class="admin-cell-meta">{{ $activity['role'] ?? '-' }}</td>
                ...
                <td class="admin-cell-meta">{{ $formatDateTime($activity['created_at'] ?? '') }}</td>
              </tr>
            @empty
    ...
    <section class="admin-card p-0 mt-5">
      ...
      <div class="admin-data-table-wrap">
        <table class="admin-table admin-data-table genbi-point-detail-table">
          <thead><tr><th>Kegiatan</th><th>Poin</th><th>Tanggal</th><th>Action</th></tr></thead>
          <tbody>
            @forelse ($manualActivities as $activity)
              <tr>
                <td class="admin-cell-title"><strong>{{ $activity['activity_name'] ?? '' }}</strong></td>
                <td class="admin-cell-meta">{{ (int) ($activity['points'] ?? 0) }} poin</td>
                <td class="admin-cell-meta">{{ $formatDate($activity['activity_date'] ?? '') }}</td>
