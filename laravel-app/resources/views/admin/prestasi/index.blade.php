...
<section class="mx-auto max-w-7xl">
  ...
  <div class="mt-6">
    ...
    <section class="admin-card p-0 mt-5"><div class="admin-data-table-wrap"><table class="admin-table admin-data-table admin-data-table-prestasi"><thead><tr><th>No.</th><th>Judul</th><th>Nama Anggota</th><th>Kategori</th><th>Tahun</th><th>Penyelenggara</th><th>Status</th><th>Action</th></tr></thead><tbody id="admin-prestasi-list" data-ssr="true">
      @forelse ($items as $index => $item)
        @php($status = strtolower($item['status'] ?? 'draft'))
        <tr><td class="admin-cell-index">{{ $startItem + $index }}</td><td class="admin-cell-title"><div class="admin-table-media">@if(!empty($item['image']))<img src="{{ $item['image'] }}" class="table-thumb rounded" alt="{{ $item['title'] }}">@else<span class="admin-thumb-placeholder">No image</span>@endif<div class="admin-table-title"><strong>{{ $item['title'] ?? '' }}</strong><p>{{ \Illuminate\Support\Str::limit($item['description'] ?? '', 90) }}</p></div></div></td><td class="admin-cell-meta">{{ $item['name'] ?? '' }}</td><td class="admin-cell-meta"><span class="cms-pill">{{ $item['category'] ?? '' }}</span></td><td class="admin-cell-year">{{ $item['year'] ?? '' }}</td><td class="admin-cell-meta">{{ $item['institution'] ?? '' }}</td><td class="admin-cell-status"><span class="cms-pill {{ $status === 'published' ? 'cms-pill-green' : ($status === 'draft' ? 'cms-pill-yellow' : '') }}">{{ ucfirst($status) }}</span></td><td class="admin-cell-actions"><div class="admin-table-actions"><button type="button" class="btn btn-outline btn-sm" data-detail-prestasi="{{ (int) $item['id'] }}">Detail</button>@if($status !== 'published')<button type="button" class="btn btn-primary btn-sm" data-approve-prestasi="{{ (int) $item['id'] }}">Approve</button>@endif<a class="btn btn-secondary btn-sm" href="{{ route('admin.prestasi.edit', ['id' => (int) $item['id']]) }}">Edit</a><button type="button" class="btn btn-danger btn-sm" data-delete-prestasi="{{ (int) $item['id'] }}">Delete</button></div></td></tr>
