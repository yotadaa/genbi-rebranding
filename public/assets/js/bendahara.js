    ...
    const renderTable = () => {
        ...
        if (currentData.length === 0) {
            ...
            currentData.forEach(t => {
                ...
                if (!isDashboard) {
                    actionHtml += `
                        <button type="button" class="btn-edit p-1.5 text-amber-500 hover:bg-amber-50 rounded-lg transition-colors ml-1" data-id="${t.id}" title="Edit">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                        </button>
                        <button type="button" class="btn-delete p-1.5 text-red-600 hover:bg-red-50 rounded-lg transition-colors ml-1" data-id="${t.id}" title="Hapus">
