(function () {
  ...
  function bindAgendaActions() {
    document.querySelectorAll('[data-agenda-delete]').forEach((button) => {
      button.addEventListener('click', async () => {
        ...
        if (!ok) return;
        const res = await fetch(route('admin.eventDelete', { id }), { method: 'POST', headers: { Accept: 'application/json', 'Content-Type': 'application/json', 'X-CSRF-TOKEN': API.getCsrfToken?.() || '' }, credentials: 'same-origin', body: JSON.stringify({ _csrf_token: API.getCsrfToken?.() || '' }) });
  ...
  async function renderEventEditor() {
    ...
    body.innerHTML = `<form class="editor-workspace" id="event-form"><main class="block-writing-surface"><div id="event-title" class="news-title-block" contenteditable="true" data-placeholder="Judul agenda...">${escape(item?.title || '')}</div><div id="event-excerpt" class="news-excerpt-block" contenteditable="true" data-placeholder="Ringkasan pendek agenda...">${escape(item?.excerpt || '')}</div><article id="event-content" class="news-body-block" contenteditable="true" data-placeholder="Deskripsi agenda...">${item?.content || ''}</article><div id="event-location" class="news-excerpt-block" contenteditable="true" data-placeholder="Lokasi agenda...">${escape(item?.location || '')}</div><input id="event-map" class="config-input mt-4" value="${escape(item?.map || '')}" placeholder="Embed map atau URL lokasi..." /></main><aside class="editor-config-sidebar"><section class="config-card"><h2>Agenda Date</h2>${control('Start Date', `<input id="event-start-date" class="config-input" type="date" value="${escape(item?.start_date || '2026-05-05')}" />`)}${control('End Date', `<input id="event-end-date" class="config-input" type="date" value="${escape(item?.end_date || '2026-05-05')}" />`)}</section><section class="config-card"><h2>Photo and Banner</h2>${control('Featured Photo URL', `<input id="event-photo" class="config-input" value="${escape(item?.photo || '')}" placeholder="/uploads/... atau https://..." />`)}${control('Banner URL', `<input id="event-banner" class="config-input" value="${escape(item?.banner || '')}" placeholder="/uploads/... atau https://..." />`)}</section><section class="config-card"><h2>SEO Information</h2>${control('Meta Title', `<input id="event-meta-title" class="config-input" value="${escape(item?.meta_title || item?.title || '')}" />`)}${control('Meta Description', `<textarea id="event-meta-description" class="config-input" rows="5">${escape(item?.meta_description || item?.excerpt || '')}</textarea>`)}</section><button type="submit" class="btn btn-primary w-full">${isEdit ? 'Update Agenda' : 'Submit Agenda'}</button></aside></form>`;
    document.querySelector('#event-form').addEventListener('submit', async (event) => {
      event.preventDefault();
      const payload = { title: document.querySelector('#event-title')?.textContent?.trim() || '', excerpt: document.querySelector('#event-excerpt')?.textContent?.trim() || '', content: document.querySelector('#event-content')?.innerHTML || '', location: document.querySelector('#event-location')?.textContent?.trim() || '', map: document.querySelector('#event-map')?.value?.trim() || '', start_date: document.querySelector('#event-start-date')?.value || '', end_date: document.querySelector('#event-end-date')?.value || '', photo: document.querySelector('#event-photo')?.value?.trim() || '', banner: document.querySelector('#event-banner')?.value?.trim() || '', meta_title: document.querySelector('#event-meta-title')?.value?.trim() || '', meta_description: document.querySelector('#event-meta-description')?.value?.trim() || '' };
      const url = isEdit ? route('admin.eventUpdate', { id: eventId }) : route('admin.eventStore');
      const res = await fetch(url, { method: 'POST', headers: { Accept: 'application/json', 'Content-Type': 'application/json', 'X-CSRF-TOKEN': API.getCsrfToken?.() || '' }, credentials: 'same-origin', body: JSON.stringify(payload) });
