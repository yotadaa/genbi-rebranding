(function(){"use strict";const{getParam:p,newsDetailUrl:$,pageUrl:k,renderShell:S}=window.GenBIApp,{observeFadeUp:C}=window.GenBIUI,l=window.GenBIAPI,c=window.GenBIAPICore;if(S("news"),C(),document.querySelector("#news-detail-root")?.dataset.ssr==="true"){L(),document.body.classList.add("page-ready");return}N(),window.addEventListener("error",v),window.addEventListener("unhandledrejection",v);function L(){g(document),w(document,{slug:window.location.pathname.split("/").filter(Boolean).pop()||""})}async function N(){const t=document.querySelector("#news-detail-root");t.innerHTML='<section class="bg-stone py-16"><div class="article-container text-sm text-neutral-600">Memuat detail berita...</div></section>';const a=p("slug")||p("id")||document.body.dataset.routeSlug||"";if(!a){t.innerHTML='<section class="bg-stone py-16"><div class="article-container text-sm text-neutral-600">Berita tidak ditemukan.</div></section>';return}let e,r,n;try{e=await l.getNewsDetail(a),r=await l.getRelatedNews(e.id,e.category),n=await l.getNewsComments(e)}catch{t.innerHTML='<section class="bg-stone py-16"><div class="article-container text-sm text-neutral-600">Gagal memuat berita. Silakan coba lagi.</div></section>';return}document.title=`${e.title} | GenBI Provinsi Jambi`,t.dataset.loaded="true",t.innerHTML=`
        ...
        <div class="news-engagement-grid">
          <section class="share-card">
            ...
            <div class="mt-6 flex flex-wrap gap-2">
              <button class="share-button" data-share-url="https://wa.me/?text=${encodeURIComponent(e.title)}%20${encodeURIComponent(c.canonicalNewsUrl(e))}">WhatsApp</button>
              <button class="share-button" data-share-url="https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(c.canonicalNewsUrl(e))}">Facebook</button>
              <button class="share-button" data-share-url="https://twitter.com/intent/tweet?text=${encodeURIComponent(e.title)}&url=${encodeURIComponent(c.canonicalNewsUrl(e))}">X</button>
              <button class="share-button" data-copy data-canonical="${c.canonicalNewsUrl(e)}">Copy Link</button>
            </div>
          </section>
        ...
        <div class="mt-6 related-news-list">
          ${t.map(a=>`
            <a data-transition href="${$(a)}" class="related-news-card">
              <figure class="news-detail-hero-img"><img class="news-detail-inline-image" src="${a.image}" alt="tes ${a.title}" loading="lazy" onerror="this.src='https://genbijambi.com/public/uploads/slider-1.png'" /></figure>
              <div>
