document.addEventListener("DOMContentLoaded",()=>{let c=[],I=[];try{const e=document.getElementById("trx-data");e&&(c=JSON.parse(e.dataset.transactions||"[]"));const t=document.getElementById("trx-categories");t&&(I=JSON.parse(t.dataset.categories||"[]"))}catch(e){console.error("Error parsing data:",e)}const b=e=>new Intl.NumberFormat("id-ID",{style:"currency",currency:"IDR",minimumFractionDigits:0}).format(e).replace("Rp","Rp "),B=e=>new Date(e).toLocaleDateString("id-ID",{day:"2-digit",month:"short",year:"numeric"}),D=document.getElementById("btn-mobile-menu"),g=document.getElementById("sidebar-menu"),h=document.getElementById("sidebar-backdrop");if(D&&g&&h){const e=()=>{g.classList.remove("translate-x-0"),g.classList.add("-translate-x-full"),h.classList.add("hidden")};D.addEventListener("click",()=>{g.classList.remove("-translate-x-full"),g.classList.add("translate-x-0"),h.classList.remove("hidden")}),h.addEventListener("click",e)}let v="Semua",S="",i=1;const C=5,k=document.getElementById("table-body"),j=document.getElementById("page-info"),w=document.getElementById("btn-prev"),E=document.getElementById("btn-next"),L=document.getElementById("filter-container"),M=document.getElementById("search-input"),$=document.getElementById("btn-add-trx"),P=document.getElementById("summary-masuk"),N=document.getElementById("summary-keluar"),A=document.getElementById("summary-saldo"),H=document.getElementById("cashflowChart"),m=()=>{if(!k)return;let e=c.filter(n=>{const s=v==="Semua"||n.category===v,r=n.desc.toLowerCase().includes(S.toLowerCase());return s&&r});e.sort((n,s)=>new Date(s.date)-new Date(n.date)),z(e),q(e);const t=e.length,a=Math.ceil(t/C)||1;i>a&&(i=a);const l=(i-1)*C,d=e.slice(l,l+C);k.innerHTML="",d.length===0?k.innerHTML=`
            ...
            `:d.forEach(n=>{const s=document.createElement("tr");s.className="hover:bg-slate-50/50 transition-colors";const r=window.location.pathname.includes("unja"),f=r?"text-orange-600":"text-blue-600",p=n.type==="in"?f:"text-rose-600",x=n.type==="in"?"+":"-",y=!document.getElementById("btn-add-trx");let o=`
                ...
                `;const u=r?"/keuangan/bendahara/unja/transaksi":"/keuangan/bendahara/wilayah/transaksi",F=r?"hapus":"delete";y||(o+=`
                        <a href="${u}/edit/${n.id}" class="btn-edit-link p-1.5 text-amber-500 hover:bg-amber-50 rounded-lg transition-colors ml-1" title="Edit">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                        </a>
                        <form action="${u}/${F}/${n.id}" method="POST" class="inline">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium ${p} text-right">
                        ${x}${b(n.amount)}
                    </td>
                ...
                `,k.appendChild(s)}),j&&(j.textContent=`Halaman ${i} dari ${a}`),w&&(w.disabled=i===1),E&&(E.disabled=i===a),document.querySelectorAll(".btn-detail").forEach(n=>{n.addEventListener("click",s=>{const r=parseInt(s.currentTarget.dataset.id,10);K(r)})}),document.querySelectorAll(".btn-edit").forEach(n=>{n.addEventListener("click",s=>{const r=parseInt(s.currentTarget.dataset.id,10);O(r)})}),document.querySelectorAll(".btn-delete").forEach(n=>{n.addEventListener("click",s=>{const r=parseInt(s.currentTarget.dataset.id,10);U(r)})})},z=e=>{if(!P||!N||!A)return;let t=0,a=0;e.forEach(l=>{l.type==="in"?t+=l.amount:a+=l.amount}),P.textContent=b(t),N.textContent=b(a),A.textContent=b(t-a)};let T=null;const q=e=>{if(!H)return;const t={};e.forEach(o=>{t[o.date]||(t[o.date]={in:0,out:0}),t[o.date][o.type]+=o.amount});const a=Object.keys(t).sort((o,u)=>new Date(o)-new Date(u)),l=a.map(o=>B(o)),d=a.map(o=>t[o].in),n=a.map(o=>t[o].out);T&&T.destroy();const s=window.location.pathname.includes("unja"),r=s?"rgba(234, 88, 12, 1)":"rgba(37, 99, 235, 1)",f=s?"rgba(249, 115, 22, 0.1)":"rgba(59, 130, 246, 0.1)",p=H.getContext("2d"),x=p.createLinearGradient(0,0,0,300);x.addColorStop(0,r),x.addColorStop(1,f);const y=p.createLinearGradient(0,0,0,300);y.addColorStop(0,"rgba(225, 29, 72, 1)"),y.addColorStop(1,"rgba(244, 63, 94, 0.1)"),T=new Chart(p,{type:"bar",data:{labels:l,datasets:[{label:"Pemasukan",data:d,backgroundColor:x,borderRadius:6,borderSkipped:!1,barThickness:16},{label:"Pengeluaran",data:n,backgroundColor:y,borderRadius:6,borderSkipped:!1,barThickness:16}]},options:{responsive:!0,maintainAspectRatio:!1,interaction:{mode:"index",intersect:!1},scales:{x:{grid:{display:!1,drawBorder:!1},ticks:{font:{family:"'Inter', sans-serif",size:12},color:"#64748b"}},y:{beginAtZero:!0,grid:{color:"rgba(226, 232, 240, 0.8)",borderDash:[4,4],drawBorder:!1},border:{display:!1},ticks:{font:{family:"'Inter', sans-serif",size:12},color:"#94a3b8",padding:10,callback:function(o){return o>=1e6?"Rp "+o/1e6+"Jt":o>=1e3?"Rp "+o/1e3+"K":"Rp "+o}}}},plugins:{legend:{position:"top",align:"end",labels:{usePointStyle:!0,boxWidth:8,boxHeight:8,font:{family:"'Inter', sans-serif",size:12,weight:"500"},color:"#475569",padding:20}},tooltip:{backgroundColor:"rgba(15, 23, 42, 0.95)",titleFont:{family:"'Inter', sans-serif",size:13,weight:"600"},bodyFont:{family:"'Inter', sans-serif",size:13},padding:12,cornerRadius:12,usePointStyle:!0,boxPadding:6,callbacks:{label:function(o){let u=o.dataset.label||"";return u&&(u+=": "),o.parsed.y!==null&&(u+=b(o.parsed.y)),u}}}}}})};L&&L.addEventListener("click",e=>{if(e.target.tagName==="BUTTON"){v=e.target.dataset.cat;const a=window.location.pathname.includes("unja")?"bg-orange-600":"bg-[#3b5998]";L.querySelectorAll("button").forEach(l=>{l.dataset.cat===v?l.className=`category-btn px-5 py-2.5 rounded-xl text-[13px] font-semibold transition-all ${a} text-white shadow-md`:l.className="category-btn px-5 py-2.5 rounded-xl text-[13px] font-medium transition-all bg-slate-50 border border-slate-200/60 text-slate-600 hover:bg-slate-100"}),i=1,m()}}),M&&M.addEventListener("input",e=>{S=e.target.value,i=1,m()}),w&&w.addEventListener("click",()=>{i>1&&(i--,m())}),E&&E.addEventListener("click",()=>{i++,m()}),$&&$.addEventListener("click",e=>{$.tagName==="BUTTON"&&(e.preventDefault(),O())});const R=(e,t)=>{const a=document.getElementById(e);a&&a.remove();const l=document.createElement("div");return l.id=e,l.className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm",l.innerHTML=t,document.body.appendChild(l),l.addEventListener("click",d=>{d.target===l&&l.remove()}),l},K=e=>{const t=c.find(s=>s.id===e);if(!t)return;const a=window.location.pathname.includes("unja"),l=a?"text-orange-600":"text-blue-600",d=a?"bg-orange-50 text-orange-700":"bg-blue-50 text-blue-700",n=`
        ...
        `;R("modal-detail",n)},O=(e=null)=>{let t=e!==null,a=t?c.find(s=>s.id===e):{date:new Date().toISOString().split("T")[0],desc:"",category:I[0]||"",type:"out",amount:""};const l=I.map(s=>`<option value="${s}" ${a.category===s?"selected":""}>${s}</option>`).join(""),d=`
