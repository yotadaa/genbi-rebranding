document.addEventListener("DOMContentLoaded",()=>{let u=[],B=[];try{const t=document.getElementById("trx-data");t&&(u=JSON.parse(t.dataset.transactions||"[]"));const e=document.getElementById("trx-categories");e&&(B=JSON.parse(e.dataset.categories||"[]"))}catch(t){console.error("Error parsing data:",t)}const g=t=>new Intl.NumberFormat("id-ID",{style:"currency",currency:"IDR",minimumFractionDigits:0}).format(t).replace("Rp","Rp "),$=t=>new Date(t).toLocaleDateString("id-ID",{day:"2-digit",month:"short",year:"numeric"}),D=document.getElementById("btn-mobile-menu"),f=document.getElementById("sidebar-menu"),v=document.getElementById("sidebar-backdrop");if(D&&f&&v){const t=()=>{f.classList.remove("translate-x-0"),f.classList.add("-translate-x-full"),v.classList.add("hidden")};D.addEventListener("click",()=>{f.classList.remove("-translate-x-full"),f.classList.add("translate-x-0"),v.classList.remove("hidden")}),v.addEventListener("click",t)}let k="Semua",M="",d=1;const C=5,w=document.getElementById("table-body"),P=document.getElementById("page-info"),E=document.getElementById("btn-prev"),I=document.getElementById("btn-next"),L=document.getElementById("filter-container"),N=document.getElementById("search-input"),T=document.getElementById("btn-add-trx"),A=document.getElementById("summary-masuk"),H=document.getElementById("summary-keluar"),R=document.getElementById("summary-saldo"),z=document.getElementById("cashflowChart"),p=()=>{if(!w)return;let t=u.filter(n=>{const s=k==="Semua"||n.category===k,r=n.desc.toLowerCase().includes(M.toLowerCase());return s&&r});t.sort((n,s)=>new Date(s.date)-new Date(n.date)),U(t),_(t);const e=t.length,a=Math.ceil(e/C)||1;d>a&&(d=a);const l=(d-1)*C,i=t.slice(l,l+C);w.innerHTML="",i.length===0?w.innerHTML=`
            ...
            `:i.forEach(n=>{const s=document.createElement("tr");s.className="hover:bg-slate-50/50 transition-colors";const r=window.location.pathname.includes("unja"),m=window.location.pathname.includes("uin"),b=r?"text-orange-600":m?"text-sky-500":"text-blue-600",x=n.type==="in"?b:"text-rose-600",h=n.type==="in"?"+":"-",y=!document.getElementById("btn-add-trx");let o=`
                ...
                `,c="/keuangan/bendahara/wilayah/transaksi",S="delete";r?(c="/keuangan/bendahara/unja/transaksi",S="hapus"):m&&(c="/keuangan/bendahara/uin/transaksi",S="hapus"),y||(o+=`
                        ...
                        <form action="${c}/${S}/${n.id}" method="POST" class="inline">
                            <input type="hidden" name="_csrf_token" value="${document.querySelector('meta[name="csrf-token"]')?document.querySelector('meta[name="csrf-token"]').getAttribute("content"):""}">
        ...
        `;O("modal-detail",m)},q=(t=null)=>{let e=t!==null,a=e?u.find(s=>s.id===t):{date:new Date().toISOString().split("T")[0],desc:"",category:B[0]||"",type:"out",amount:""};const l=B.map(s=>`<option value="${s}" ${a.category===s?"selected":""}>${s}</option>`).join(""),i=`
