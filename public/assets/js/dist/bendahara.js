document.addEventListener("DOMContentLoaded",()=>{let d=[],y=[];try{const e=document.getElementById("trx-data");e&&(d=JSON.parse(e.dataset.transactions||"[]"));const t=document.getElementById("trx-categories");t&&(y=JSON.parse(t.dataset.categories||"[]"))}catch(e){console.error("Error parsing data:",e)}const m=e=>new Intl.NumberFormat("id-ID",{style:"currency",currency:"IDR",minimumFractionDigits:0}).format(e).replace("Rp","Rp "),h=e=>new Date(e).toLocaleDateString("id-ID",{day:"2-digit",month:"short",year:"numeric"}),L=document.getElementById("btn-mobile-menu"),p=document.getElementById("sidebar-menu"),b=document.getElementById("sidebar-backdrop");if(L&&p&&b){const e=()=>{p.classList.remove("translate-x-0"),p.classList.add("-translate-x-full"),b.classList.add("hidden")};L.addEventListener("click",()=>{p.classList.remove("-translate-x-full"),p.classList.add("translate-x-0"),b.classList.remove("hidden")}),b.addEventListener("click",e)}let f="Semua",$="",r=1;const k=5,g=document.getElementById("table-body"),C=document.getElementById("page-info"),x=document.getElementById("btn-prev"),v=document.getElementById("btn-next"),w=document.getElementById("filter-container"),D=document.getElementById("search-input"),E=document.getElementById("btn-add-trx"),T=document.getElementById("summary-masuk"),M=document.getElementById("summary-keluar"),S=document.getElementById("summary-saldo"),N=document.getElementById("cashflowChart"),u=()=>{if(!g)return;let e=d.filter(n=>{const o=f==="Semua"||n.category===f,a=n.desc.toLowerCase().includes($.toLowerCase());return o&&a});e.sort((n,o)=>new Date(o.date)-new Date(n.date)),H(e),O(e);const t=e.length,s=Math.ceil(t/k)||1;r>s&&(r=s);const l=(r-1)*k,c=e.slice(l,l+k);g.innerHTML="",c.length===0?g.innerHTML=`
                <tr>
                    <td colspan="5" class="px-6 py-12 text-center text-slate-500 text-sm">Belum ada data transaksi.</td>
                </tr>
            `:c.forEach(n=>{const o=document.createElement("tr");o.className="hover:bg-slate-50/50 transition-colors";const a=n.type==="in"?"text-blue-600":"text-rose-600",i=n.type==="in"?"+":"-",B=!document.getElementById("btn-add-trx");let A=`
                ...
                `,g.appendChild(o)}),C&&(C.textContent=`Halaman ${r} dari ${s}`),x&&(x.disabled=r===1),v&&(v.disabled=r===s),document.querySelectorAll(".btn-detail").forEach(n=>{n.addEventListener("click",o=>{const a=parseInt(o.currentTarget.dataset.id,10);R(a)})}),document.querySelectorAll(".btn-edit").forEach(n=>{n.addEventListener("click",o=>{const a=parseInt(o.currentTarget.dataset.id,10);P(a)})}),document.querySelectorAll(".btn-delete").forEach(n=>{n.addEventListener("click",o=>{const a=parseInt(o.currentTarget.dataset.id,10);q(a)})})},H=e=>{if(!T||!M||!S)return;let t=0,s=0;e.forEach(l=>{l.type==="in"?t+=l.amount:s+=l.amount}),T.textContent=m(t),M.textContent=m(s),S.textContent=m(t-s)};let I=null;const O=e=>{if(!N)return;const t={};e.forEach(a=>{t[a.date]||(t[a.date]={in:0,out:0}),t[a.date][a.type]+=a.amount});const s=Object.keys(t).sort((a,i)=>new Date(a)-new Date(i)),l=s.map(a=>h(a)),c=s.map(a=>t[a].in),n=s.map(a=>t[a].out);I&&I.destroy();const o=N.getContext("2d");I=new Chart(o,{type:"bar",data:{labels:l,datasets:[{label:"Pemasukan",data:c,backgroundColor:"#2563eb",borderRadius:4},{label:"Pengeluaran",data:n,backgroundColor:"#e11d48",borderRadius:4}]},options:{responsive:!0,maintainAspectRatio:!1,scales:{y:{beginAtZero:!0,ticks:{callback:function(a){return"Rp "+a/1e6+"Jt"}}}},plugins:{legend:{position:"top"},tooltip:{callbacks:{label:function(a){let i=a.dataset.label||"";return i&&(i+=": "),a.parsed.y!==null&&(i+=m(a.parsed.y)),i}}}}}})};w&&w.addEventListener("click",e=>{e.target.tagName==="BUTTON"&&(f=e.target.dataset.cat,w.querySelectorAll("button").forEach(t=>{t.dataset.cat===f?t.className="category-btn px-4 py-2 rounded-lg text-sm font-medium transition-all bg-blue-600 text-white shadow-md shadow-blue-200":t.className="category-btn px-4 py-2 rounded-lg text-sm font-medium transition-all bg-blue-50 text-blue-700 hover:bg-blue-100"}),r=1,u())}),D&&D.addEventListener("input",e=>{$=e.target.value,r=1,u()}),x&&x.addEventListener("click",()=>{r>1&&(r--,u())}),v&&v.addEventListener("click",()=>{r++,u()}),E&&E.addEventListener("click",e=>{E.tagName==="BUTTON"&&(e.preventDefault(),P())});const j=(e,t)=>{const s=document.getElementById(e);s&&s.remove();const l=document.createElement("div");return l.id=e,l.className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm",l.innerHTML=t,document.body.appendChild(l),l.addEventListener("click",c=>{c.target===l&&l.remove()}),l},R=e=>{const t=d.find(l=>l.id===e);if(!t)return;const s=`
        ...
        `;j("modal-detail",s)},P=(e=null)=>{let t=e!==null,s=t?d.find(o=>o.id===e):{date:new Date().toISOString().split("T")[0],desc:"",category:y[0]||"",type:"out",amount:""};const l=y.map(o=>`<option value="${o}" ${s.category===o?"selected":""}>${o}</option>`).join(""),c=`
